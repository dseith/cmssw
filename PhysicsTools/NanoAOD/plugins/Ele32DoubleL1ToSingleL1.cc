...
class Ele32DoubleL1ToSingleL1 : public edm::global::EDProducer<>{
private:
...
public:
  explicit Ele32DoubleL1ToSingleL1(const edm::ParameterSet& iPara);
};

Ele32DoubleL1ToSingleL1::Ele32DoubleL1ToSingleL1(const edm::ParameterSet& iPara):
...
void Ele32DoubleL1ToSingleL1::produce(edm::StreamID streamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const
{ 
  ...
  for(auto& ele : *elesHandle){
    ...
    pass_ele_32.push_back(pass);
  }

  std::unique_ptr<edm::ValueMap<bool>> passEle32(new edm::ValueMap<bool>());
  edm::ValueMap<bool>::Filler fillPass(*passEle32);
  fillPass.insert(elesHandle, pass_ele_32.begin(), pass_ele_32.end());
  fillPass.fill();
  iEvent.put(std::move(passEle32));
}
