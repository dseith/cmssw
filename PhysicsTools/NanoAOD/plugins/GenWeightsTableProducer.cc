...
class GenWeightsTableProducer : public edm::global::EDProducer<edm::StreamCache<Counter>, edm::RunCache<DynamicWeightChoice>, edm::RunSummaryCache<Counter>, edm::EndRunProducer> {
    public:
                ...
                std::unique_ptr<nanoaod::FlatTable> & outPS ) const
        {
            ...
            int vectorSize = (genProd.weights().size() == 14 || genProd.weights().size() == 46) ? 12 : 1;
            ...
            outPS->addColumn<float>("", wPS, vectorSize > 1 ? "PS weights (w_var / w_nominal); [0] is ISR=1/sqrt(2) FSR=1; [1] is ISR=1 FSR=1/sqrt(2); [2] is ISR=sqrt(2) FSR=1; [3] is ISR=1 FSR=sqrt(2) [4] is ISR=0.5 FSR=1; [5] is ISR=1 FSR=0.5; [6] is ISR=2 FSR=1; [7] is ISR=1 FSR=2 [8] is ISR=0.25 FSR=1; [9] is ISR=1 FSR=0.25; [10] is ISR=4 FSR=1; [11] is ISR=1 FSR=4" : "dummy PS weight (1.0) ", nanoaod::FlatTable::FloatColumn, lheWeightPrecision_);
                ...
                std::unique_ptr<nanoaod::FlatTable> & outPS ) const
        {
            ...
            if (vectorSize > 1 ) {
                for (unsigned int i=2; i<14; i++){
                    wPS[i] = (genProd.weights()[i])/genWeight;
                }
            }
            outPS.reset(new nanoaod::FlatTable(wPS.size(), "PSWeight", false));
            outPS->addColumn<float>("", wPS, vectorSize > 1 ? "PS weights (w_var / w_nominal); [0] is ISR=1/sqrt(2) FSR=1; [1] is ISR=1 FSR=1/sqrt(2); [2] is ISR=sqrt(2) FSR=1; [3] is ISR=1 FSR=sqrt(2) [4] is ISR=0.5 FSR=1; [5] is ISR=1 FSR=0.5; [6] is ISR=2 FSR=1; [7] is ISR=1 FSR=2 [8] is ISR=0.25 FSR=1; [9] is ISR=1 FSR=0.25; [10] is ISR=4 FSR=1; [11] is ISR=1 FSR=4" : "dummy PS weight (1.0) ", nanoaod::FlatTable::FloatColumn, lheWeightPrecision_);
