...
class PFJetIDSelectionFunctor : public Selector<pat::Jet>  {
 ...
 public: // interface
  ...
  enum Version_t { FIRSTDATA, RUNIISTARTUP, WINTER16, WINTER17, WINTER17PUPPI, N_VERSIONS , WINTER18};
 ...
 PFJetIDSelectionFunctor( edm::ParameterSet const & params )
 {
   ...
   initCuts();
    ...
    if(version_ == WINTER18){
      ...
      if ( params.exists("NEF_FW") ) set("NEF_FW", params.getParameter<double> ("NEF_FW") );
      if ( params.exists("nNeutrals_FW") ) set("nNeutrals_FW", params.getParameter<int> ("nNeutrals_FW") );
      if ( params.exists("NHF_26_27") ) set("NHF_26_27", params.getParameter<double> ("NHF_26_27") );
      if ( params.exists("NEF_26_27") ) set("NEF_26_27", params.getParameter<double> ("NEF_26_27") );
      if ( params.exists("NCH_26_27") ) set("NCH_26_27", params.getParameter<int> ("NCH_26_27") );

      if ( quality_ == TIGHTLEPVETO ) {
          if ( params.exists("MUF") ) set("MUF", params.getParameter<double> ("MUF") );
          if ( params.exists("MUF_26_27") ) set("MUF_26_27", params.getParameter<double> ("MUF_26_27") );
          if ( params.exists("CEF_26_27") ) set("CEF_26_27", params.getParameter<double> ("CEF_26_27") );
      }
    }
  ...
  version_(version), quality_(quality)
 {
  ...
  {
    if ( version_ == FIRSTDATA || version_ == RUNIISTARTUP || version_ == WINTER16 || version_ == WINTER17 || version_ == WINTER17PUPPI || version_ == WINTER18) {
  ...
  {
    if ( version_ == FIRSTDATA || version_ == RUNIISTARTUP || version_ == WINTER16 || version_ == WINTER17 || version_ == WINTER17PUPPI || version_ == WINTER18 ){ return firstDataCuts( jet, ret, version_);
    }
    else {
      return false;
    }
		      ...
		      pat::strbitset & ret, Version_t version_)
  {
   ...
   // Cuts for |eta| < 2.4 for FIRSTDATA, RUNIISTARTUP, WINTER16 and WINTER17
    if(version_ != WINTER18 && ((version_ != WINTER17 && version_ != WINTER17PUPPI) ||  quality_ != TIGHT) ) {
    ...
    if(version_ == FIRSTDATA){// Cuts for all eta for FIRSTDATA
    ...
    else if(version_ == WINTER18){
      // Cuts for |eta| <= 2.6 for WINTER18 scenario
      ...
      if ( ignoreCut(indexCHF_)           || ( chf > cut(indexCHF_, double()) || std::abs(jet.eta()) > 2.6 ) ) passCut( ret, indexCHF_);
      if ( ignoreCut(indexNCH_)           || ( nch > cut(indexNCH_, int())    || std::abs(jet.eta()) > 2.6 ) ) passCut( ret, indexNCH_);
      if ( quality_ == TIGHTLEPVETO ) {
          if ( ignoreCut(indexMUF_)           || ( muf < cut(indexMUF_, double()) || std::abs(jet.eta()) > 2.6 ) ) passCut( ret, indexMUF_);
          if ( ignoreCut(indexCEF_)           || ( cef < cut(indexCEF_, double()) || std::abs(jet.eta()) > 2.6 ) ) passCut( ret, indexCEF_);
      }
      //Cuts for 2.6 < |eta| <= 2.7 for WINTER18 scenario
      if ( ignoreCut(indexNEF_26_27_)        || ( nef < cut(indexNEF_26_27_, double())  || std::abs(jet.eta()) <= 2.6 || std::abs(jet.eta()) > 2.7)  ) passCut( ret, indexNEF_26_27_);
      if ( ignoreCut(indexNHF_26_27_)        || ( nhf < cut(indexNHF_26_27_, double())  || std::abs(jet.eta()) <= 2.6 || std::abs(jet.eta()) > 2.7)  ) passCut( ret, indexNHF_26_27_);
      if ( ignoreCut(indexNCH_26_27_)        || ( chf > cut(indexNCH_26_27_, double())  || std::abs(jet.eta()) <= 2.6 || std::abs(jet.eta()) > 2.7)  ) passCut( ret, indexNCH_26_27_);
      if ( quality_ == TIGHTLEPVETO) {
          if ( ignoreCut(indexMUF_26_27_)           || ( muf < cut(indexMUF_26_27_, double()) || std::abs(jet.eta()) <= 2.6 || std::abs(jet.eta()) > 2.7) ) passCut( ret, indexMUF_26_27_);
          if ( ignoreCut(indexCEF_26_27_)           || ( cef < cut(indexCEF_26_27_, double()) || std::abs(jet.eta()) <= 2.6 || std::abs(jet.eta()) > 2.7) ) passCut( ret, indexCEF_26_27_);
      }
      // Cuts for 2.7 < |eta| <= 3.0 for WINTER18 scenario

      if ( ignoreCut(indexNEF_EC_L_)        || ( nef > cut(indexNEF_EC_L_, double())  || std::abs(jet.eta()) <= 2.7 || std::abs(jet.eta()) > 3.0)  ) passCut( ret, indexNEF_EC_L_);
      if ( ignoreCut(indexNEF_EC_U_)        || ( nef < cut(indexNEF_EC_U_, double())  || std::abs(jet.eta()) <= 2.7 || std::abs(jet.eta()) > 3.0)  ) passCut( ret, indexNEF_EC_U_);  
      if ( ignoreCut(indexNNeutrals_EC_)  || ( nneutrals > cut(indexNNeutrals_EC_, int())  || std::abs(jet.eta()) <= 2.7 || std::abs(jet.eta()) > 3.0)  ) passCut( ret, indexNNeutrals_EC_);

      // Cuts for |eta| > 3.0 for WINTER18 scenario
      if ( ignoreCut(indexNHF_FW_)           || ( nhf > cut(indexNHF_FW_, double()) || std::abs(jet.eta()) <= 3.0 ) ) passCut( ret, indexNHF_FW_);
      if ( ignoreCut(indexNEF_FW_)           || ( nef < cut(indexNEF_FW_, double()) || std::abs(jet.eta()) <= 3.0 ) ) passCut( ret, indexNEF_FW_);
      if ( ignoreCut(indexNNeutrals_FW_) || ( nneutrals > cut(indexNNeutrals_FW_, int())    || std::abs(jet.eta()) <= 3.0 ) ) passCut( ret, indexNNeutrals_FW_);
 ...
 private: // member variables
  ...
  void initCuts()
  {
    push_back("CHF" );
    push_back("NHF" );
    if( (version_ != WINTER17 && version_!=WINTER17PUPPI && version_ != WINTER18) || quality_ != TIGHT ) push_back("CEF" );
    push_back("NEF" );
    push_back("NCH" );
    ...
    if( (version_ == WINTER17 || version_ == WINTER17PUPPI || version_ == WINTER18) && quality_ == LOOSE ){
	...
	set("nNeutrals_FW",10);
      ...
      set("CHF", 0.0);
      set("NHF", 0.9);
      if(version_ != WINTER17 && version_ != WINTER17PUPPI && version_ != WINTER18) set("CEF", 0.99);
      set("NEF", 0.9);
      set("NCH", 0);
      set("nConstituents", 1);
      if(version_ == RUNIISTARTUP){
  ...
  void initIndex()
  {
    ...
    if((version_ != WINTER17 && version_ != WINTER17PUPPI && version_ != WINTER18) || quality_ != TIGHT ) indexCEF_ = index_type (&bits_, "CEF");
    ...
    if(version_ == WINTER18){
      ...
      indexNNeutrals_FW_ = index_type (&bits_, "nNeutrals_FW");
      if ( quality_ == TIGHTLEPVETO ) {indexMUF_ = index_type (&bits_, "MUF");}
      indexNEF_26_27_ = index_type(&bits_, "NEF_26_27");
      indexNHF_26_27_ = index_type(&bits_, "NHF_26_27");
      indexNCH_26_27_ = index_type(&bits_, "NCH_26_27");
      if(quality_ == TIGHTLEPVETO) {
          indexMUF_26_27_ = index_type(&bits_, "MUF_26_27");
          indexCEF_26_27_ = index_type(&bits_, "CEF_26_27");
      }
    }
  ...
  index_type indexNEF_26_27_;
  index_type indexNHF_26_27_;
  index_type indexNCH_26_27_;
  index_type indexMUF_26_27_;
  index_type indexCEF_26_27_;
};
